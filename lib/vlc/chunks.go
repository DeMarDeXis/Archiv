...
// BinaryChunks - slice of BinaryChunk
...
// NewBinChunks creates BinaryChunks from byte slice.
...
// NewBinChunk creates BinaryChunk from byte.
func NewBinChunk(code byte) BinaryChunk {
	return BinaryChunk(fmt.Sprintf("%08b", code))
}

// Bytes returns byte slice from BinaryChunks.
...
// Byte returns byte from BinaryChunk.
...
func splitByChunks(bStr string, chunkSize int) BinaryChunks {
	strLen := utf8.RuneCountInString(bStr) //ALT len() чтоб кириллица читалась
